...
def run_grid_on_image(fn, path, out_dir, sp_list, sr_list, canny_low, canny_high, test_scale=512):
    img = cv2.imread(path)
    if img is None:
    ...
    cv2.imwrite(os.path.join(img_out_folder, f'{base_name}_orig_resized.png'), img_small)
    ...
    for sp in sp_list:
        for sr in sr_list:
            ...
            tag = f'sp{sp}_sr{sr}'
            cv2.imwrite(os.path.join(img_out_folder, f'{base_name}_ms_{tag}.png'), filtered)
            cv2.imwrite(os.path.join(img_out_folder, f'{base_name}_ms_{tag}_edges.png'), edges)
            ...
            results.append({
                ...
                'ms_edges': edge_count,
                'jaccard': float(jac),
                'out_filtered': os.path.join(img_out_folder, f'{base_name}_ms_{tag}.png'),
                'out_edges': os.path.join(img_out_folder, f'{base_name}_ms_{tag}_edges.png'),
            })

    return results


def apply_config_to_full_image(path, out_path_filtered, out_path_edges, sp, sr, canny_low, canny_high):
    img = cv2.imread(path)
    if img is None:
        print('Failed to load', path)
        return False
...
def main():
    ...
    for fn, path in imgs1:
        ...
        res = run_grid_on_image(fn, path, out_group1, sp_list, sr_list, canny_low, canny_high)
        rows.extend(res)
        if res:
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['image', 'sp', 'sr', 'orig_edges', 'ms_edges', 'jaccard', 'out_filtered', 'out_edges'])
        for r in rows:
            writer.writerow([r['image'], r['sp'], r['sr'], r['orig_edges'], r['ms_edges'], r['jaccard'], r['out_filtered'], r['out_edges']])
    ...
    with open(applied_csv, 'w', newline='', encoding='utf-8') as f:
        ...
        for fn, path in imgs1:
            base_name = os.path.splitext(fn)[0]
            outf = os.path.join(out_group1, f'{base_name}_ms_sp{chosen_sp}_sr{chosen_sr}.png')
            oute = os.path.join(out_group1, f'{base_name}_ms_sp{chosen_sp}_sr{chosen_sr}_edges.png')
            ok = apply_config_to_full_image(path, outf, oute, chosen_sp, chosen_sr, canny_low, canny_high)
            if ok:
        ...
        if os.path.exists(group2):
            ...
            for fn, path in imgs2:
                base_name = os.path.splitext(fn)[0]
                outf = os.path.join(out_group2, f'{base_name}_ms_sp{chosen_sp}_sr{chosen_sr}.png')
                oute = os.path.join(out_group2, f'{base_name}_ms_sp{chosen_sp}_sr{chosen_sr}_edges.png')
                ok = apply_config_to_full_image(path, outf, oute, chosen_sp, chosen_sr, canny_low, canny_high)
                if ok:
